...
- `audit/fe-tabs-manager-audit-2026-05-08.md` — single-file audit of `public/js/modules/tabs-manager.js` (562 lines). Resolves five Phase 2 puzzles: tabs-manager fires exactly one fetch per click and has no client-side cache (premise of "four-request fan-out from per-feature-manager iteration" displaced — source remains unidentified, follow-up audit needed); `setOriginalQuery` has zero definitions across `public/`, `pages/`, `lib/` (the `integration.js:696–702` branch is dead); `handleDomChanges` re-binds click listeners on late-arriving containers and does not fetch; tab/facet coupling is server-driven via the M1 MutationObserver cascade against the rendered DOM; tab-click requests bypass `lib/cache.ts` entirely and the `tab:` namespace is populated by `pages/api/search.ts` form=partial substring fallback only. Two latent fragilities catalogued: initial-load selector-overlap double-bind in `findTabContainers`, and listener-leakage in `this.tabContainers` across innerHTML replacements.

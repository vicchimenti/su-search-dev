...
> **[Amendment notice — 2026-05-11.]** This audit was amended on 2026-05-11 following the network recap re-examination addendum (`audit/network-recap-re-examination-2026-05-11-addendum.md`), which produced runtime evidence (DevTools Event Listeners panel inspection) that displaces two framings in this document. The original text is preserved as a record of how thinking developed; the amendments are flagged in-line with `[Amended 2026-05-11 per addendum]` annotation blocks at the end of (a) Phase 1.2 listener inventory, and (b) Phase 2 Puzzle 3's "only structurally observable double-fetch source" claim. Readers consulting this audit for current understanding should read the original framing plus the amendment annotations together; the corrected framing is in the annotations. The headline correction is that the four-request fan-out is produced by *layered listener attachment paths across lifecycle moments*, not by selector overlap; the L105 initial-load forEach never fires in production because `findTabContainers()` returns empty at module-init time (DOM not yet rendered).
...
**[Amended 2026-05-11 per addendum — lifecycle topology of the three attachment sites.]**

The three-attachment enumeration above is structurally correct but lifecycle-incomplete. Per the 2026-05-11 runtime capture (DevTools Event Listeners panel inspection on a live tab click in the dev environment), the actual runtime topology is:

- **Attachment 1 (L105 — container-level forEach in `installTabClickHandlers`) — never fires in production.** `installTabClickHandlers()` runs once at module-init (via `initialize` at L70). At module-init, `findTabContainers()` runs against a DOM that has not yet rendered the tab containers; the four `document.querySelectorAll` calls all return empty NodeLists; `this.tabContainers.length` is 0; the `if (this.tabContainers.length > 0)` guard at L103 evaluates false; the **else** branch fires instead. The forEach at L105 is structurally reachable in code but unreachable in the production lifecycle. The selector-overlap hazard described in attachment 1's idempotency paragraph is therefore structural-only — it cannot fire because the loop it would fire in never executes.

- **Attachment 2 (L109 — document-level fallback) — fires once at module-init.** The else branch wins. A single document-level click delegate is attached. This listener is never demoted or removed — there is no demotion path in the file. It remains attached for the lifetime of the page.

- **Attachment 3 (L534 — late-added container delegation in `handleDomChanges`) — fires on every M1 mutation that adds matching tab-container nodes.** When the search results render via the initial `/api/search` response, M1 detects added `#results` children matching the four container selectors, and `handleDomChanges` attaches per-container listeners on each match. The 2026-05-11 capture observed three such containers in the live DOM (a `.tabs` element plus two ancestors matching `[role="tablist"]` and `.tab-container` through the rendered template's nesting); three listeners attach.

**Runtime topology:** one document-level listener (attachment 2) plus three per-container listeners (attachment 3), all reachable for any bubbling tab click. A click on a tab anchor inside the rendered container fires `handleTabClick` four times — once at each container level on the way up, plus once at document level. Each invocation runs `loadTabContent` → `fetchFromProxy`, producing four byte-identical fetches per click.

The footnote in attachment 2 of the original enumeration anticipated *some* of this ("the document-level fallback stays attached"; "a click that bubbles up from a tab inside one of those late-added containers would then run both the container-level listener and the document-level listener — handling the same click twice"). What the original framing missed is that (a) attachment 1 never fires at all, so the audit's reasoning was operating with the wrong set of live attachments, and (b) the addendum's runtime capture shows the per-container path adds *three* listeners (one per matching ancestor), not one — accounting for four fetches per click, not two. The forwarded analytics dedup at L267 absorbs the duplicate analytics fires, but `loadTabContent`'s `fetchFromProxy` call is not deduped at the manager level; the four fetches all leave the FE.

See `audit/network-recap-re-examination-2026-05-11-addendum.md` for the full runtime evidence, the Event Listeners panel inspection, and the deferred-DOM-availability defect class.
...
**[Amended 2026-05-11 per addendum — the "only structurally observable double-fetch source" claim is incorrect.]**

The Phase 2 Puzzle 3 resolution above identified initial-load selector overlap on `findTabContainers` (L79–L96) as "the only structurally observable double-fetch source in `tabs-manager.js`." Per the 2026-05-11 runtime capture, this claim is incorrect on two counts.

First, the initial-load forEach at L105 — the loop that would attach duplicate listeners under selector overlap — never fires in production. `installTabClickHandlers()` runs at module-init when `findTabContainers()` returns empty (the tab DOM has not yet rendered), so `this.tabContainers.length > 0` is false and the else branch (L109 document-level fallback) wins instead. The selector-overlap hazard described in this puzzle is structural-only — `findTabContainers` could in principle push the same node twice, but the resulting `this.tabContainers` is never iterated by an attachment path that fires.

Second, there is a much larger structural double-fetch source than the one this puzzle identified: **layered listener attachment paths across lifecycle moments.** The L109 document-level fallback (attached at module-init when no DOM exists) is never removed when M1-driven `handleDomChanges` later attaches per-container listeners via L534 on the rendered DOM. The two paths coexist at runtime; the runtime capture observed one document-level listener plus three per-container listeners on three nested ancestors matching the four container selectors. Every tab click bubbles through all four listeners, firing `handleTabClick` four times, producing four `fetchFromProxy` calls per click.

The corrected framing: the structural defect is not selector overlap within a single attachment path, it is **deferred-DOM-availability across lifecycle moments**. `installTabClickHandlers()` falls back to a document-level delegate as a hedge against the DOM not being ready at module-init time, but does not remove the hedge when M1 confirms the DOM has arrived and re-attaches per-container listeners. The two phases of the lifecycle produce additive listener counts rather than alternating ones, because the early-phase hedge has no demotion path.

This amendment does not invalidate the "Items raised by Puzzle 3 for the rebuild" recommendations above — they still apply, and the deferred-DOM-availability finding *strengthens* the case for the second bullet (event delegation from a stable parent with a clear single-attachment-strategy lifecycle). The third recommendation about tracking containers by stable identity remains valuable but is now secondary to the lifecycle-design point.

See `audit/network-recap-re-examination-2026-05-11-addendum.md` for the runtime evidence, the Event Listeners panel inspection, and the cadence-item-6-closed-without-firing note. See `funnelback-search-project-instructions-v15.md` §"Resolved in v14" for the fan-out closure framing and the resulting "layered listener attachment strategies across lifecycle moments" anti-pattern that this audit's amendment also reinforces.
